...
func createRC(stage *models.Stage) error {
	...
	if _, err := k8s.ReplicationControllers(stage.Namespace).Create(rc); err != nil {
...
func deleteRC(stage *models.Stage) error {
	...
	return k8s.ReplicationControllers(stage.Namespace).Delete(stage.Name)
}
...
func checkRC(stage *models.Stage) (bool, error) {
	...
	rcs, err := k8s.ReplicationControllers(stage.Namespace).List(api.ListOptions{})
	if err != nil {
		return false, nil
	}
...
func watchRCStatus(stage *models.Stage, checkOp string, ch chan *models.K8SRes) {
	if err := checkClient(); err != nil {
	...
	w, err := k8s.ReplicationControllers(stage.Namespace).Watch(opts)
	if err != nil {
		ch <- formatResult(models.ResultFailed, err.Error())
		return
	}
